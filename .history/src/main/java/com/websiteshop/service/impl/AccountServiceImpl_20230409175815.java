...
@Service
public class AccountServiceImpl<BufferedImage> implements AccountService {
    @Autowired
    AccountDAO adao;
    ...
    public void loginFormOAuth2(OAuth2AuthenticationToken oauth2) {
        ...
        // Lấy URL của hình ảnh đại diện từ thuộc tính "picture"
        ...
        account.setImage(pictureUrl); // thêm ảnh
        ...
        try {
            // Tạo đối tượng URL từ URL của hình ảnh
            URL url = new URL(pictureUrl);

            // Tải hình ảnh từ URL và lưu vào đối tượng BufferedImage
            java.awt.image.BufferedImage image = ImageIO.read(url);

            // Hiển thị hình ảnh trong ứng dụng của bạn
            // Ví dụ: Hiển thị trong một JLabel
            ImageIcon icon = new ImageIcon(image);
            JLabel label = new JLabel(icon);
            // Thêm label vào frame hoặc container của bạn
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
