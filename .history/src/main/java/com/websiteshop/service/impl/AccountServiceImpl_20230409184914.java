...
public class AccountServiceImpl implements AccountService {
    ...
    // Luu vào CSDL
    @Override
    public void loginFormOAuth2(OAuth2AuthenticationToken oauth2) throws IOException {
        ...
        String pictureUrl = oauth2.getPrincipal().getAttribute("picture");

        // Lấy đuôi của tập tin ảnh từ URL
        String extension = pictureUrl.substring(pictureUrl.lastIndexOf("."));

        // Tạo tên ngẫu nhiên cho tập tin ảnh
        String randomName = UUID.randomUUID().toString() + extension;

        // Tạo đối tượng File để lưu tập tin ảnh
        File file = new File("/path/to/save/images/" + randomName);

        // Tải tập tin ảnh từ URL và lưu vào đối tượng File
        URL url = new URL(pictureUrl);
        FileUtils.copyURLToFile(url, file);

        // Đổi tên tập tin sang tên mới ngẫu nhiên
        File newFile = new File("/path/to/save/images/" + UUID.randomUUID().toString() + ".jpg");
        file.renameTo(newFile);

        // Lấy đường dẫn của tập tin mới để lưu vào CSDL
        String picturePath = newFile.getAbsolutePath();
        ...
        account.setImage(picturePath); // thêm ảnh
